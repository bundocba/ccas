...
class modVT_Nivo_SliderHelper
{
	...
	{
		$db = &JFactory::getDbo();
		$query = "SELECT * FROM #__fw_nivogallery WHERE published = 1 
	...
	public static function getSlider($params, $separator = "\n"){
		...
		foreach($items as $i=>$value){
			...
			//$link	= trim($link);
			$link	= htmlspecialchars($value['link'], ENT_QUOTES);
			...
			//$desc	= trim($desc);
			$desc	= strip_tags($value['description'],"<a><br><p>");

			$path = JURI::base(true).DS.$value['image'];
