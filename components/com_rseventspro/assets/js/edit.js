window.addEvent('domready', function(){
	// Autocompleter
	if (isset($('frontend'))) {
		new Autocompleter.Request.HTML('rs_location', 'index.php?option=com_rseventspro&task=locations', {
			'postVar': 'rs_location',
			minLength: 1,
			maxChoices: 100,
			autoSubmit: false,
...
/* tags */
	...
	myList = new TextboxList('rs_tags',{
				bitsOptions:{
					editable:{
						addKeys: 188
					}
	...
	myList2 = new TextboxList('meta_key',{
				bitsOptions:{
					editable:{
						addKeys: 188
					}
...
function rs_edit_addticket(unlimited, buttontext, error, remove, confirmmsg) {
	...
	var req = new Request({
		...
		onSuccess: function(responseText, responseXML) {
			...
			if (isset($('rs_li_td')))
				theli.inject($('rs_li_td'), 'before');
			else
				theli.inject($('rs_li_9'), 'before');
	...
	if (isset($('frontend')))
		req.send('task=rseventspro.saveticket&jform[ide]=' + eventID + '&jform[name]='+ ticket_name + '&jform[price]='+ ticket_price + '&jform[seats]=' + ticket_seats + '&jform[user_seats]='+ ticket_user_seats + '&jform[description]='+ ticket_description + '&randomTime='+Math.random());
	else
		req.send('task=savedata&type=ticket&jform[ide]=' + eventID + '&jform[name]='+ ticket_name + '&jform[price]='+ ticket_price + '&jform[seats]=' + ticket_seats + '&jform[user_seats]='+ ticket_user_seats + '&jform[description]='+ ticket_description);
}
