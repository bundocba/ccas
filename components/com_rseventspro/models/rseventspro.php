...
{	
	...
	protected function _buildQuery() {
		...
		if ($parent && $counter) {
			...
			if (empty($from) && !empty($to)) {
				...
				if (!empty($includeTo)) {
					$where[] = ' AND ('.$this->_db->qn('e.end').' <= '.$this->_db->q($to).' OR '.$this->_db->qn('e.id').' IN ('.implode(',',$includeTo).')) ';
				} else {
					$where[] = ' AND '.$this->_db->qn('e.end').' <= '.$this->_db->q($to).' ';
				}
				...
				if (!empty($includeFrom)) {
					$where[] = ' AND ('.$this->_db->qn('e.start').' >= '.$this->_db->q($from).' OR '.$this->_db->qn('e.id').' IN ('.implode(',',$includeFrom).')) ';
				} else {
					$where[] = ' AND '.$this->_db->qn('e.start').' >= '.$this->_db->q($from).' ';
				}
			} elseif (!empty($from) && !empty($to)) {
				...
				if (!empty($includeFromTo)) {
					$where[] = ' AND ((('.$this->_db->qn('e.start').' <= '.$this->_db->q($from).' AND '.$this->_db->qn('e.end').' >= '.$this->_db->q($to).') OR ('.$this->_db->qn('e.start').' >= '.$this->_db->q($from).' AND '.$this->_db->qn('e.end').' <= '.$this->_db->q($to).')) OR '.$this->_db->qn('e.id').' IN ('.implode(',',$includeFromTo).')) ';
				} else {
					$where[] = ' AND (('.$this->_db->qn('e.start').' <= '.$this->_db->q($from).' AND '.$this->_db->qn('e.end').' >= '.$this->_db->q($to).') OR ('.$this->_db->qn('e.start').' >= '.$this->_db->q($from).' AND '.$this->_db->qn('e.end').' <= '.$this->_db->q($to).')) ';
				}
			}
		...
		$query .= ' ORDER BY '.$this->_db->qn('e.'.$order).' '.$this->_db->escape($direction).' ';
	...
	protected function _buildLocationQuery() {
		...
		$query->clear()
			->select($this->_db->qn('id'))->select($this->_db->qn('name'))->select($this->_db->qn('description'))
			->from($this->_db->qn('#__rseventspro_locations'))
			->where($this->_db->qn('published').' = 1')
			->order($this->_db->qn('name').' '.$this->_db->escape($order));
	...
	protected function _buildCategoriesQuery() {
		...
		$query->clear()
			->select($this->_db->qn('id'))->select($this->_db->qn('title'))->select($this->_db->qn('description'))
	...
	protected function _buildSearchQuery() {
		...
		$query->clear()
			...
			->order($this->_db->qn($order).' '.$this->_db->escape($direction));
		...
		if (!empty($search)) {
			$search = $this->_db->Quote('%'.$this->_db->escape($search, true).'%');
			$query->where('('.$this->_db->qn('e.name').' LIKE '.$search.' OR '.$this->_db->qn('e.description').' LIKE '.$search.' OR '.$this->_db->qn('l.name').' LIKE '.$search.' OR '.$this->_db->qn('l.description').' LIKE '.$search.' OR '.$this->_db->qn('l.address').' LIKE '.$search.')');
		}
	...
	public function getSubscriber() {
		...
		$query->clear()
			...
			->select($this->_db->qn('ticket_pdf'))->select($this->_db->qn('ticket_pdf_layout'))
			->from($this->_db->qn('#__rseventspro_events'))
	...
	public function subscribe($idsubmission = null) {
		...
		$query->clear()
			...
			->select($this->_db->qn('automatically_approve'))->select($this->_db->qn('notify_me'))->select($this->_db->qn('owner'))
			->from($this->_db->qn('#__rseventspro_events'))
			->where($this->_db->qn('id').' = '.$id);
		...
		if (!empty($tickets)) {
			foreach ($tickets as $tid => $quantity) {
				...
				if ($ticket->price > 0) {
					$info .= $quantity . ' x ' .$ticket->name.' ('.rseventsproHelper::currency($ticket->price).')<br />';
				} else {
