...
{	
	...
	protected function _buildQuery() {
		...
		if (($this->_app->input->get('layout') == 'day' || $this->_app->input->get('tpl') == 'day') && !empty($date)) {
			...
			if (!empty($includeDay)) {
				$query .= ' AND '.$this->_db->qn('e.end').' <> '.$this->_db->q($this->_db->getNullDate()).' ';
				$query .= ' AND ((('.$this->_db->qn('e.start').' <= '.$this->_db->q($start).' AND '.$this->_db->qn('e.end').' >= '.$this->_db->q($start).') OR ('.$this->_db->qn('e.start').' >= '.$this->_db->q($start).' AND '.$this->_db->qn('e.start').' <= '.$this->_db->q($end).')) OR '.$this->_db->qn('e.id').' IN ('.implode(',',$includeDay).')) ';
			} else {
		...
		if (($this->_app->input->get('layout') == 'week' || $this->_app->input->get('tpl') == 'week') && !empty($date)) {
			...
			if (!empty($includeWeek)) {
				$query .= ' AND '.$this->_db->qn('e.end').' <> '.$this->_db->q($this->_db->getNullDate()).' ';
				$query .= ' AND ((('.$this->_db->qn('e.start').' <= '.$this->_db->q($start).' AND '.$this->_db->qn('e.end').' >= '.$this->_db->q($start).') OR ('.$this->_db->qn('e.start').' >= '.$this->_db->q($start).' AND '.$this->_db->qn('e.start').' <= '.$this->_db->q($end).')) OR '.$this->_db->qn('e.id').' IN ('.implode(',',$includeWeek).')) ';
			} else {
				$query .= ' AND (('.$this->_db->qn('e.start').' <= '.$this->_db->q($start).' AND '.$this->_db->qn('e.end').' >= '.$this->_db->q($start).') OR ('.$this->_db->qn('e.start').' >= '.$this->_db->q($start).' AND '.$this->_db->qn('e.start').' <= '.$this->_db->q($end).')) ';
			}
		}
