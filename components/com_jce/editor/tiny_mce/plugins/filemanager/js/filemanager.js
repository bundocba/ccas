    ...
    var FileManager = {
        ...
        init: function() {
            ...
            if (el != null && ed.dom.is(el, 'a.jce_file, a.wf_file')) {
                ...
                var options = $('li', '#layout_options_list').get();
                var ordered = [];
                    ...
                    switch(n.nodeName) {
                    	case 'IMG':
                    		if (ed.dom.is(n, '.jce_icon, .wf_file_icon')) {
                    			$('#layout_option_icon_check').prop('checked', true);
                    			ordered.push($('#layout_option_icon').get(0));
                    		}
                    		break;
                    	case '#text':
                    		if (/[\w]+/i.test(n.data)) {
                    			$('#layout_option_text_check').prop('checked', true);
                    			
                    			$('#text').val(n.data);
                        		ordered.push($('#layout_option_text').get(0));
                    		}
                    		break;
                    	case 'SPAN':
                    		...
                    		if (ed.dom.is(n, '.jce_size, .jce_file_size, .wf_file_size')) {
                    			$('input[type="text"]', '#layout_option_size').val(v);
                        		$('#layout_option_size_check').prop('checked', true);
                        		$('#size_class').val(tinymce.trim(cls));
                        		ordered.push($('#layout_option_size').get(0));
                    		}
                    		
                    		if (ed.dom.is(n, '.jce_date, .jce_file_date, .wf_file_date')) {
                    			$('input[type="text"]', '#layout_option_date').val(v);
                        		$('#layout_option_date_check').prop('checked', true);
                        		$('#date_class').val(tinymce.trim(cls));
                        		ordered.push($('#layout_option_date').get(0));
                    		}
                ...
                $('#layout_options_list').append(ordered);               
            } else {
            ...
            $('#layout_options_enabled').toggle(sortables);
            $('#layout_options_disabled').toggle(!sortables);
        ...
        insertAndClose: function() {
            ...
            var options 	= $('#layout_options_list').sortable('toArray');
            ...
            if (icon.charAt(0) == '/') {
           	...
           	var data = {
                ...
                date: '<span class="wf_file_date" style="margin-left:5px;">' + $('input:text', '#layout_option_date').val() + '</span>',
                size: '<span class="wf_file_size" style="margin-left:5px;">' + $('input:text', '#layout_option_size').val() + '</span>',
            ...
            $.each(options, function(i, v) {
                if ($('input:checkbox', '#' + v).is(':checked')) {
                    html.push(data[v.replace('layout_option_', '')]);
                }
            });
        ...
        _setupSortables: function() {
            ...
            // add event
            $('li', '#layout_options_list').click( function(e) {
                ...
                if ($(el).is(':checkbox:checked, span.layout_option_reload')) {
                    
                    if ($(el).is(':checkbox') && $(el).siblings('input:text').val()) {
                    	return;
                    }
                    
                    if ($(p).is('#layout_option_size, #layout_option_date') && items.length) {
        ...
        selectFile : function(file) {
            ...
            // size
            $('input:text', '#layout_option_size').val($.String.formatSize($(file).data('size')));
            // date
            $('input:text', '#layout_option_date').val($.String.formatDate($(file).data('modified'), this.settings.date_format));
