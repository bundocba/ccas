...
class WFImageManagerPlugin extends WFMediaManager {
	...
	function getPresetsList($type) {
		...
		switch ($type) {
			case 'resize' :
				$list = explode(',', $this->getParam('imgmanager_ext.resize_presets', '320x240,640x480,800x600,1024x768'));
				break;
			case 'crop' :
				$list = explode(',', $this->getParam('imgmanager_ext.crop_presets', '4:3,16:9,20:30,320x240,240x320,640x480,480x640,800x600,1024x768'));
				break;
		}
	...
	function onUpload($file) {
		...
		if($dim) {
			...
			if($resize) {
				$rw = JRequest::getInt('upload_resize_width', $params->get('imgmanager_ext.resize_width', 640));
				$rh = JRequest::getInt('upload_resize_height', $params->get('imgmanager_ext.resize_height', 480));
				$rq = JRequest::getInt('upload_resize_quality', $params->get('imgmanager_ext.resize_quality', 80));
				...
				if(!$resized) {					
					if ($this->checkMem($w * $h)) {
						if(!$editor->resize($file, '', $rw, $rh, $rq)) {
			...
			if($thumbnail) {
				...
				$tq = JRequest::getInt('upload_thumbnail_quality', $params->get('imgmanager_ext.thumbnail_quality', 80));
			...
			/*if ($rotate) {
			 $ra = JRequest::getInt('upload_rotate_angle', $params->get('imgmanager_ext.rotate_angle', '90'));
			 if (!$editor->rotate($file, $ra)) {
			 $browser->setResult(WFText::_('WF_IMGMANAGER_EXT_ROTATE_ERROR'), 'error');
			 }
			 }*/
		}
	...
	function getThumbnail($relative) {
		...
		$dir = WFUtility::makePath(str_replace("\\", "/", dirname($relative)), $params->get('imgmanager_ext.thumbnail_folder', 'thumbnails'));
		$thumbnail = WFUtility::makePath($dir, $this->getThumbName($relative));

		// Image is a thumbnail
		if(strpos($relative, $params->get('imgmanager_ext.thumbnail_prefix', 'thumb_'))) {
			return $relative;
		}
		//the original image is smaller than thumbnails,
		//so just return the url to the original image.
		if($dim[0] <= $params->get('imgmanager_ext.thumbnail_size', '150') && $dim[1] <= $params->get('imgmanager_ext.thumbnail_size', '150')) {
			return $relative;
		}
	...
	function getThumbName($file) {
		return $this->getParam('imgmanager_ext.thumbnail_prefix', 'thumb_') . basename($file);
	}
	...
	function getThumbDir($file, $create) {
		...
		if($create && !$filesystem->exists($dir)) {
			$browser->folderNew(dirname($dir), basename($dir));
		}
	...
	function deleteThumbnail($file) {
		...
		if($browser->deleteItem($this->getThumbPath($file))) {
			if(!$filesystem->countFiles($dir) && !$filesystem->countFolders($dir)) {
	...
	function cleanCacheDir() {
		...
		$cache_max_size 	= intval($params->get('imgmanager_ext.cache_size', 10)) * 1024 * 1024;
		$cache_max_age 		= intval($params->get('imgmanager_ext.cache_age', 30)) * 86400;
		$cache_max_files 	= intval($params->get('imgmanager_ext.cache_files', 0));
	...
	function getSettings() {
		...
		$settings = array(
			...
			'attributes' => array(
				'dimensions' => $params->get('imgmanager_ext.attributes_dimensions', 1), 
				'align' => $params->get('imgmanager_ext.attributes_align', 1), 
				'margin' => $params->get('imgmanager_ext.attributes_margin', 1), 
				'border' => $params->get('imgmanager_ext.attributes_border', 1)
			), 
			'view_mode' 		=> $params->get('imgmanager_ext.mode', 'list'), 
			'canEdit' 			=> $this->canEdit(),
			'cache_enable' 		=> $params->get('imgmanager_ext.cache_enable', 0)
		);
	...
	function getDefaults() {
		...
		$defaults = array(
			// Upload
			'upload_resize' 		=> $params->get('imgmanager_ext.upload_resize', 1), 
			'upload_resize_state' 	=> $params->get('imgmanager_ext.upload_resize_state', 0), 
			'upload_resize_width' 	=> $params->get('imgmanager_ext.resize_width', 640), 
			'upload_resize_height' 	=> $params->get('imgmanager_ext.resize_height', 480), 
			'resize_quality' 		=> $params->get('imgmanager_ext.resize_quality', 100),
			// Thumbnails
			'upload_thumbnail' 			=> $params->get('imgmanager_ext.upload_thumbnail', 1), 
			'upload_thumbnail_state' 	=> $params->get('imgmanager_ext.upload_thumbnail_state', 0), 
			'thumbnail_width' 			=> $params->get('imgmanager_ext.thumbnail_width', 120), 
			'thumbnail_height' 			=> $params->get('imgmanager_ext.thumbnail_height', 90), 
			'thumbnail_quality' 		=> $params->get('imgmanager_ext.thumbnail_quality', 80)
		);
